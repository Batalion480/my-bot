...
class PDFGenerator:
    ...
    def generate(self, data: Dict[str, Any]) -> bytes:
        ...
        if not positions:
            ...
            # Реквизиты КП из первой позиции (они одинаковы для всех)
            kp_numbers = positions[0].get('kp_numbers', ['', '', ''])
            kp_dates = positions[0].get('kp_dates', ['', '', ''])

            # Заголовки КП (объединённые на две колонки)
            kp_headers = []
            for i in range(3):
            ...
            # Первая строка заголовка (общие колонки + объединённые КП)
            header_row1 = [
                "№ п/п",
            ...
            # Вторая строка (подзаголовки для КП: цена за ед. и общая цена)
            sub_header_row = [
                "", "", "", "", "",
                "Цена за ед.\nизм.",
                "Цена",
            ...
            # Заметим, что теперь у нас 11 колонок: №, наименование, ОКПД2, кол-во, ед.изм., КП1_ед, КП1_общ, КП2_ед, КП2_общ, КП3_ед, КП3_общ, средняя, вариация, итого.
            # Но в первом ряду мы объединяем КП1 на две колонки, КП2 на две, КП3 на две.
            # В подзаголовках мы заполняем соответствующие колонки.

            # Поэтому пересчитаем колонки: всего 14 колонок: 5 общих + 2*3 = 6 для КП + 3 для средней, вариации, итого = 5+6+3 = 14.
            # Сделаем так, чтобы заголовок первой строки был длиной 14, а вторая строка тоже 14.
            # Перестроим заголовки.

            # Упростим: сделаем как в примере: общие колонки (5) + для каждого КП две колонки (2*3=6) + итого 3 колонки (средняя, вариация, итого) = 14.
            # Но в примере (файл 3) в шапке есть "Коммерческое предложение от ..." и под ним две колонки, так и сделаем.
            ...
            header_row1 = [
                ...
                "Кол-во",
                "Ед. изм.",
                kp_headers[0],  # объединяется на две колонки
                kp_headers[1],  # объединяется на две колонки
                kp_headers[2],  # объединяется на две колонки
                "Средняя цена\nза ед. (руб.)",
                "Коэф.\nвариации (%)",
                "Итого (руб.)"
            ]
            # Теперь количество колонок: 5 + 3 (объединённых, но каждая занимает 2 колонки) + 3 = 5+6+3 = 14.
            # Но в header_row1 мы указали только 11 элементов, потому что мы объединяем каждые два столбца под одним заголовком.
            # Для этого мы будем использовать SPAN в TableStyle.

            # Вместо сложных манипуляций, проще построить таблицу с 14 колонками и объединять ячейки.
            # Создадим список для первой строки: сначала 5 общих, затем для каждого КП по 2 колонки (всего 6), затем 3 итоговых.
            # Для первой строки мы объединим каждые две колонки для КП.
            first_row = [
                "№ п/п",
                "Наименование позиции",
                "ОКПД2/КТРУ",
            ...
            # Длина: 5 + 2*3 + 3 = 14, но у нас сейчас 5 + 6 + 3 = 14, но элементы для КП: kp_headers[0], "" и т.д. – это 6 элементов.
            # Всего: 5 (первые) + 6 (КП) + 3 (последние) = 14.
            # Это правильно.
            ...
            # Длина: 5 пустых + 6 подзаголовков (по две на КП) + 3 пустых = 14.

            # Теперь формируем данные таблицы
            ...
            for idx, pos in enumerate(positions, 1):
                ...
                # Для каждой позиции создаём строку: общие данные + для каждого КП две ячейки (цена за ед. и общая цена)
                row = [
                    str(idx),
                    pos.get('name', ''),
                    pos.get('okpd', ''),
                ...
                for price in prices:
                    total = price * qty
                    row.append(f"{price:.2f}")  # цена за ед.
                    row.append(f"{total:.2f}")  # общая цена
            ...
            # Итоговая строка (только для общей цены)
            total_row = ["", "", "", "", "", "", "", "", "", "", "", "", "ИТОГО:", f"{total_nmck:.2f}"]
            table_data.append(total_row)

            # Ширина колонок: 5 общих + 6 для КП + 3 итоговых = 14 колонок
            col_widths = [
                0.8*cm,   # №
                4.5*cm,   # Наименование
                2.8*cm,   # ОКПД2
            ...
            # Объединяем ячейки для КП в первой строке: каждая пара колонок (6,7), (8,9), (10,11) объединяется
            table.setStyle(TableStyle([
                ...
                # Объединяем ячейки для КП в первой строке (индексы колонок с 0)
                ('SPAN', (5, 0), (6, 0)),  # КП1
                ('SPAN', (7, 0), (8, 0)),  # КП2
                ('SPAN', (9, 0), (10, 0)), # КП3
                ('BACKGROUND', (0, 1), (-1, 1), colors.lightgrey),
                ...
                # Для колонок "Средняя", "Вариация", "Итого" в первой строке объединять не нужно — они по одной колонке
            ]))
            story.append(table)
            story.append(Spacer(1, 12))
